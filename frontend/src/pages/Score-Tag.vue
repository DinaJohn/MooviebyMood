...
export default defineComponent({
  ...
  setup() {
    ...
    const nextSlide = () => {
      if (currentIndex.value < recommendations.value.length + 1) { // ensure it doesn't exceed the number of slides
        currentIndex.value++;
      } else {
        currentIndex.value = 0; 
      }
    ...
    const prevSlide = () => {
      if (currentIndex.value > 0) {
        ...
        currentIndex.value = recommendations.value.length + 1; // wrap to the last slide
      }    
    };
...
/* Hover styles for each mood button */
...
.movie-overview{
  display: flex;
  flex-direction: column;
  width: 50vw; /* Maximum width of the carousel */
  font-size: 2em;
}
